...
import project.guide.anu.travellanka.adapterpackage.MyCityListAdapter;
...
//        gpsTracker = new GPSLocation(ActivityCitySelection.this);
  ...
  * this method used to download the data file in given link **/
       ...
       if(downloadPosition==0){
         ...
         final DownloadTask downloadTask = new DownloadTask(ActivityCitySelection.this,mProgressDialog,fileName);
       if(downloadPosition==0) {
           downloadTask.execute("https://www.dropbox.com/s/lo0qj65howsx8fm/colomboNEW.txt?dl=0");
       }
      else {
           downloadTask.execute("https://www.dropbox.com/s/k8mhm5rfgbdfwjt/gallesam.txt?dl=0");
       }
